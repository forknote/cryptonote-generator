...
if [[ ${FILE_cryptonote_config} == *\#define\ *CRYPTONOTE_NAME\ *\""${__CONFIG_core_CRYPTONOTE_NAME}"\"* ]]
then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX\ *"${__CONFIG_core_CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX}"* ]]
then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *P2P_DEFAULT_PORT\ *"${__CONFIG_core_P2P_DEFAULT_PORT}"* ]]
then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *RPC_DEFAULT_PORT\ *"${__CONFIG_core_RPC_DEFAULT_PORT}"* ]]
then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *P2P_STAT_TRUSTED_PUB_KEY\ *\""${__CONFIG_core_P2P_STAT_TRUSTED_PUB_KEY}"\"* ]]
then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *MONEY_SUPPLY\ *"${__CONFIG_core_MONEY_SUPPLY}"* ]]; then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *EMISSION_SPEED_FACTOR\ *\("${__CONFIG_core_EMISSION_SPEED_FACTOR}"\)* ]]; then
	echo "TEST PASSED - EMISSION_SPEED_FACTOR change"
else
	echo "TEST FAILED - EMISSION_SPEED_FACTOR change"
	exit 2
...
if [[ ${FILE_cryptonote_config} == *\#define\ *DIFFICULTY_TARGET\ *"${__CONFIG_core_DIFFICULTY_TARGET}"* ]]; then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *COIN\ *"${__CONFIG_core_COIN}"* ]]; then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *CRYPTONOTE_DISPLAY_DECIMAL_POINT\ *"${__CONFIG_core_CRYPTONOTE_DISPLAY_DECIMAL_POINT}"* ]]; then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *DEFAULT_FEE\ *"${__CONFIG_core_MINIMUM_FEE}"* ]]; then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW\ *"${__CONFIG_core_CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW}"* ]]
then
...
if [[ ${FILE_cryptonote_config} == *\#define\ *CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE\ *"${__CONFIG_core_CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE}"* ]]
then
...
if [[ ${FILE_p2p_networks} == *const\ static\ boost::uuids::uuid\ CRYPTONOTE_NETWORK\ *\=\ *{\ {\ "${__CONFIG_core_BYTECOIN_NETWORK}"}\ }* ]]
then
