...
GENESIS_COINBASE_TX_HEX_re = re.compile(r"(const char\s+GENESIS_COINBASE_TX_HEX\[\]\s+=) \"\S*\";", re.IGNORECASE)
...
if 'MINIMUM_FEE' in config['core']:
    MINIMUM_FEE_re = re.compile(r"(const uint64_t MINIMUM_FEE\s+=)\s+\S*", re.IGNORECASE)
...
for line in fileinput.input([paths['cryptonote_config']], inplace=True):
    ...
    if 'MONEY_SUPPLY' in config['core']:
        line = MONEY_SUPPLY_re.sub("\\1 UINT64_C(%s);" % config['core']['MONEY_SUPPLY'], line)
    if 'EMISSION_SPEED_FACTOR' in config['core']:
    ...
    if 'MINIMUM_FEE' in config['core']:
        line = MINIMUM_FEE_re.sub("\\1 UINT64_C(%s);" % int(config['core']['MINIMUM_FEE']), line)
    if 'DEFAULT_DUST_THRESHOLD' in config['core']:
...
SEED_NODES_re = re.compile(r"(const std::initializer_list<const char*>\s+SEED_NODES = {)[^;]+(};)", re.DOTALL)
CHECKPOINTS_re = re.compile(r"(const std::initializer_list<CheckpointData> \s+CHECKPOINTS = {)[^;]+(};)", re.DOTALL)
